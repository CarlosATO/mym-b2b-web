...
BEGIN
  ...
  -- ----------------------------------------------------------------------------
  v_max_items := COALESCE(max_items, 20);
  ...
  IF NOT EXISTS (
    ...
    WHERE r.id = import_run_id AND r.company_id = target_company_id
  ) THEN
  ...
  IF EXISTS (
    ...
    WHERE a.company_id = target_company_id AND a.import_run_id = import_run_id
  ) THEN
  ...
  INSERT INTO web_b2b.bsale_product_apply_runs (
    ...
    target_company_id, import_run_id, 'script', 'controlled_apply', 'running', v_max_items
  )
  ...
  LOOP
    ...
    IF EXISTS (
      ...
      WHERE p.company_id = target_company_id AND p.sku = v_item.sku
    ) THEN
      ...
      INSERT INTO web_b2b.bsale_product_apply_items (
        ...
        v_apply_run_id, target_company_id, import_run_id, v_item.id,
    ...
    IF EXISTS (
      SELECT 1 FROM web_b2b.products p
      WHERE p.company_id = target_company_id AND p.bsale_variant_id = v_item.bsale_variant_id
    ) THEN
      v_conflicts := v_conflicts + 1;
      INSERT INTO web_b2b.bsale_product_apply_items (
        apply_run_id, company_id, import_run_id, import_item_id,
    ...
    v_slug := web_b2b.generate_unique_product_slug_for_import(
      target_company_id, v_item.source_name, v_item.sku
    );
    ...
    INSERT INTO web_b2b.bsale_product_apply_items (
      ...
      v_apply_run_id, target_company_id, import_run_id, v_item.id, v_product_id,
