...
- [ ] Fase 6D.4B: Migración/RPC de apply controlado (borrador SQL, sin ejecutar). Borrador local creado: `docs/productos/borrador_apply_control_6d4b.sql` (tablas apply_runs/apply_items, helper slug, RPC system). Revisión 2 aplicada: `bsale_last_checked_at=NULL`, `max_items` con COALESCE, slug con acentos (translate), revokes del helper, duplicados como `conflict`, excepción sin candidatos, amarre FK `(id, run_id, company_id)`. Pendiente de conversión a migración formal. (en revisión)
