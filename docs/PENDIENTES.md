...
- [ ] Fase 6D.4C: Migración formal candidata + prueba técnica con ROLLBACK. Creada `supabase/migrations/20260804120000_web_b2b_controlled_bsale_product_apply.sql`. Probada solo con `BEGIN; ... ROLLBACK`: migración aplica, RPC crea máx 20 productos seguros sin persistir; negativas PASS (max_items>20, NULL→20, run inexistente, doble apply). Hallazgo: colisión de nombre `import_run_id` (42702) corregida con prefijos `p_` en la candidata y el borrador 6D.4B fue alineado con la misma corrección (sin divergencia documental). Reporte `docs/productos/REPORTE_ROLLBACK_APPLY_BSALE_6D4C.md`. Sin persistencia, sin commit. (en revisión)
